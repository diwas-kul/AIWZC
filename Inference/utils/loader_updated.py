...
class FullSequenceDataset(Dataset):
    ...
    def _load_subject_data(self):
        ...
        """
        base_path_data = './motionbert/data/'
        path_imu = './motionbert/results_test_pose/'
...
class APDataset(Dataset):
    """
    Load the AlphaPose data. Commented out code is for ground truth loading
    """
    ...
    def _load_subject_data(self):
        ...
        """
        base_path_data = '/scratch/leuven/365/vsc36565/motionbert/data/test/' if self.test else '/scratch/leuven/365/vsc36565/motionbert/data/'
        path_imu = '/scratch/leuven/365/vsc36565/motionbert/results_test_pose/demo/' if self.test else '/scratch/leuven/365/vsc36565/motionbert/results_test_pose/'
        ...
        for subject in self.subjects:
            ...
            subject_poses = []
            #train_filenames = []
            filename = '/alphapose-results.npy'
            ...
            # Load AlphaPose data (.npy) for the file
            #data = np.load(path_imu + subject + '/AlphaPose_' + filename[:-5] + '.npy', allow_pickle=True)
            data = np.load(path_imu + filename, allow_pickle=True)
        ...
        total_windows = len(all_windows[0])
