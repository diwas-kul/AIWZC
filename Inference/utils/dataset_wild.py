...
def read_input(json_path, vid_size, scale_range, focus, out_path):
    ...
    for count, item in enumerate(results):
        if item['image_id'] == prev_frame:
            double_frames.append(prev_frame)
            continue
        kpts = np.array(item['keypoints']).reshape([-1,3])
        kpts_all.append(kpts)
        prev_frame = item['image_id']
    ...
    #np.save('results/' + json_path.split('/')[-1][:-5], np.array(double_frames)) 
    np.save('%s/%s.npy' % (out_path, "double_frames"), np.array(double_frames))
    return motion.astype(np.float32)

class WildDetDataset(Dataset):
    def __init__(self, json_path, out_path, clip_len=243, vid_size=None, scale_range=None, focus=None):
        self.json_path = json_path
        self.clip_len = clip_len
        self.vid_all = read_input(json_path, vid_size, scale_range, focus, out_path)
