...
from models.SPPE.src.utils.img import load_image, cropBox, im_to_torch
from alphapose_args import opt
from models.yolo.preprocess import prep_image, prep_frame, inp_to_image
from utils.pPose_nms import pose_nms, write_json
from utils.matching import candidate_reselect as matching
from models.SPPE.src.utils.eval import getPrediction, getMultiPeakPrediction
from models.yolo.util import write_results, dynamic_write_results
...
class DetectionLoader:
    def __init__(self, dataloder, batchSize=1, queueSize=1024):
        # initialize the file video stream along with the boolean
        # used to indicate if the thread should be stopped or not
        self.det_model = Darknet("./models/yolo/cfg/yolov3-spp.cfg")
...
class DataWriter:
    def __init__(self, save_video=False,
                savepath='examples/res/1.avi', fourcc=cv2.VideoWriter_fourcc(*'XVID'), fps=30, frameSize=(640,480),
                queueSize=1):
        if save_video:
            # initialize the file video stream along with the boolean
            # used to indicate if the thread should be stopped or not
